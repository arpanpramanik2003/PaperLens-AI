...
import { Search, Loader2, Play, RefreshCw, Wand2, Globe, Database, ShieldCheck, CornerDownLeft } from "lucide-react";
...
export const AgentGoalInput: React.FC<AgentGoalInputProps> = ({
  ...
  return (
    <div className="relative rounded-2xl border border-border/80 bg-card/90 p-5 md:p-6 shadow-xl backdrop-blur-xl transition-all space-y-4 focus-within:ring-2 focus-within:ring-indigo-500/30 focus-within:border-indigo-500/80">
      {/* Input Header Bar */}
      ...
      <Textarea
        ...
        className="min-h-[110px] md:min-h-[125px] rounded-xl bg-background/50 border-border/60 p-4 text-sm md:text-base leading-relaxed text-foreground placeholder:text-muted-foreground/60 focus-visible:ring-0 focus-visible:outline-none resize-y transition-all"
      />
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pt-1 border-t border-border/40">
        {/* Connected Tools & Pipeline Capabilities */}
        <div className="hidden md:flex flex-wrap items-center gap-2 text-[11px] text-muted-foreground">
          <span className="font-mono text-[10px] uppercase font-semibold text-muted-foreground/70">Connected Tools:</span>
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-secondary/40 border border-border/40 font-mono">
            <Globe className="w-3 h-3 text-cyan-400" /> Semantic Scholar & Crossref
          </span>
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-secondary/40 border border-border/40 font-mono">
            <Database className="w-3 h-3 text-indigo-400" /> Supabase pgvector
          </span>
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-secondary/40 border border-border/40 font-mono">
            <ShieldCheck className="w-3 h-3 text-emerald-400" /> Self-Critique
          </span>
        </div>
