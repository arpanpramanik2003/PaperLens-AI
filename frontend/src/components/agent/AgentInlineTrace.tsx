...
const TOOL_LABELS: Record<string, string> = {
  ...
  search_workspace_vector_db: "Workspace Vector Search",
  analyze_paper: "Paper Deep Analysis",
  detect_gaps: "Research Gap Detection",
  generate_problem: "Problem Statement Formulation",
  find_datasets: "Dataset & Benchmark Discovery",
  plan_experiment: "Experimental Roadmap Design",
};

interface AgentInlineTraceProps {
  trace?: TraceData;
  onInspectTool?: (toolName: string) => void;
}

export const AgentInlineTrace: React.FC<AgentInlineTraceProps> = ({
  trace,
  onInspectTool,
}) => {
  ...
  return (
    <div className="space-y-2.5 my-2.5 font-sans">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-foreground/90">
                {isExecuting ? "Reasoning & Action Plan..." : "Reasoning Process"}
              </span>
          ...
          <div className="space-y-1.5">
            {receipts.map((rcpt, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="rounded-xl border border-border/60 bg-card/80 p-2.5 text-xs transition-all shadow-sm"
                >
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="font-mono text-[9px] text-muted-foreground bg-secondary/80 px-1.5 py-0.2 rounded border border-border/50">
                            {rcpt.tool}
                          </span>
                        </div>
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      ...
                      {onInspectTool && isCompleted && (
                        <button
                          ...
                          className="px-2 py-0.5 text-[10px] rounded-md bg-secondary/60 hover:bg-indigo-500/10 hover:text-indigo-300 border border-border/60 text-muted-foreground font-mono transition-colors flex items-center gap-1"
                        >
                  ...
                  {/* Expanded receipt details (arguments & output preview) */}
                  {isExpanded && (
                    <div className="mt-2.5 pt-2 border-t border-border/40 space-y-1.5 text-[10px] font-mono bg-secondary/20 p-2 rounded-lg">
                      ...
                      {rcpt.data && (
                        <div>
                          <span className="text-muted-foreground block font-semibold">Observation Payload:</span>
                          <pre className="text-foreground/80 whitespace-pre-wrap overflow-x-auto max-h-32">
                            {JSON.stringify(rcpt.data, null, 2).slice(0, 500)}
                            {JSON.stringify(rcpt.data).length > 500 ? "\n..." : ""}
                          </pre>
                        </div>
                      )}
                    </div>
